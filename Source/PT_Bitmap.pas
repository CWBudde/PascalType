...
type
  ...
  public
    ...
    procedure Draw(PixelMap: TPtCustomBitmap; Alpha: Byte = $FF); overload; virtual;
    procedure Draw(PixelMap: TPtCustomBitmap; X, Y: Integer); overload; virtual;
    procedure Draw(PixelMap: TPtCustomBitmap; X, Y: Integer; Alpha: Byte); overload; virtual;
    procedure DrawTransparent(PixelMap: TPtCustomBitmap); overload; virtual;
    procedure DrawTransparent(PixelMap: TPtCustomBitmap; X, Y: Integer); overload; virtual;
...
procedure TPtCustomBitmap.DrawTransparent(PixelMap: TPtCustomBitmap; X,
  Y: Integer);
var
  ClipRect : TRect;
  Index    : Integer;
...
procedure TPtCustomBitmap.Draw(PixelMap: TPtCustomBitmap;
  Alpha: Byte = $FF);
begin
...
begin
 ...
 Result := False;
 {$ENDIF}
 if Obj is TPtCustomBitmap
  then Result := Equal(TPtCustomBitmap(Obj));
end;
...
begin
 ...
 if HasData and (Filer.Ancestor <> nil)
  then HasData := not ((Filer.Ancestor is TPtCustomBitmap) and
...
procedure TPtCustomBitmap.FillRect(Left, Top, Right, Bottom: Integer; Color: TPixel32);
var
  X, Y : Integer;
begin
 if Color.A = $FF then
...
procedure TPtCustomBitmap.Line(FromX, FromY, ToX, ToY: Integer; Color: TPixel32);
var
  x, y, t     : Integer;
  dx, dy      : Integer;
  incx, incy  : Integer;
