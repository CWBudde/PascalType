...
type
  ...
  TBlendMergePixelInplace = procedure(Foreground: TPixel32;
    var Background: TPixel32);
  TBlendMergePixelLine = procedure(Foreground: TPixel32; Destination: PPixel32;
    Count: Cardinal);
  TBlendMergeLine = procedure(Source, Destination: PPixel32; Count: Cardinal);
  TCombinePixel = function(Foreground, Background: TPixel32; Weight: Cardinal)
    : TPixel32;
  TCombinePixelInplace = procedure(Foreground: TPixel32;
    var Background: TPixel32; Weight: Integer);
  TCombinePixelLine = procedure(Foreground: TPixel32; Destination: PPixel32;
    Count: Integer; Weight: Cardinal);
  TCombineLine = procedure(Source, Destination: PPixel32; Count: Integer;
    Weight: Cardinal);
...
{$IFDEF WIN_COLOR_FIX}
  ...
  if I <> 0 then
    Result := Result or TPixel32(Integer(I) - 1);
  I := Color and $0000FF00;
  if I <> 0 then
    Result := Result or TPixel32(Integer(I) - $00000100);
  I := Color and $000000FF;
  if I <> 0 then
    Result := Result or TPixel32(Integer(I) - 1) shl 16;
{$ELSE}
{$IFDEF USENATIVECODE}
...
{$IFNDEF TARGET_x86}
begin
  Result := ((Color.ARGB and $00FF0000) shr 16) or (Color.ARGB and $0000FF00) or
...
begin
  ...
  begin
    ...
    Foreground.ARGB := ((((Alpha * (Foreground.ARGB and $00FF00FF)) + CBias) and
      $FF00FF00) shr 8) or
      ((Alpha * ((Foreground.ARGB and $FF00FF00) shr 8) + CBias) and $FF00FF00);

    Alpha := Alpha xor $FF;

    Background.ARGB := ((((Alpha * (Background.ARGB and $00FF00FF)) + CBias) and
      $FF00FF00) shr 8) or
...
procedure BlendPixelInplaceNative(Foreground: TPixel32;
  var Background: TPixel32);
{$IFDEF PUREPASCAL}
var
  Alpha: Byte;
...
procedure BlendPixelLineNative(Foreground: TPixel32; Destination: PPixel32;
  Count: Integer);
{$IFDEF PUREPASCAL}
begin
  while Count > 0 do
...
function CombinePixelNative(Foreground, Background: TPixel32; Weight: Cardinal)
  : TPixel32;
{$IFDEF PUREPASCAL}
begin
  if Weight = 0 then
  ...
  begin
    Foreground.ARGB := (((Weight * ((Foreground.ARGB and $FF00FF00) shr 8)) +
      CBias) and $FF00FF00) +
      ((((Weight * (Foreground.ARGB and $00FF00FF)) + CBias) and
      $FF00FF00) shr 8);

    Weight := Weight xor $000000FF;
    Background.ARGB := (((Weight * ((Background.ARGB and $FF00FF00) shr 8)) +
      CBias) and $FF00FF00) +
      ((((Weight * (Background.ARGB and $00FF00FF)) + CBias) and
      $FF00FF00) shr 8);
...
procedure CombinePixelInplaceNative(Foreground: TPixel32;
...
begin
  ...
  begin
    ...
    Result.R := Background.R + (Scale * ($FF + Foreground.R - Background.R) +
      $7F) shr 8 + 1 - Scale;
    Result.G := Background.G + (Scale * ($FF + Foreground.G - Background.G) +
      $7F) shr 8 + 1 - Scale;
    Result.B := Background.B + (Scale * ($FF + Foreground.B - Background.B) +
      $7F) shr 8 + 1 - Scale;
  end;
end;
...
function CombinePixelMMX(Foreground, Background: TPixel32; Weight: TPixel32)
  : TPixel32;
asm
  {$IFDEF CPUx86_64}
  MOVD      MM1, ECX
...
procedure BlendPixelLineSSE2(Foreground: TPixel32; Destination: PPixel32;
  Count: Integer);
asm
  {$IFDEF CPUx86_64}
  TEST      R8D, R8D
...
function CombinePixelSSE2(Foreground, Background: TPixel32; Weight: TPixel32)
  : TPixel32;
asm
  {$IFDEF CPUx86_64}
  MOVD      XMM1, ECX
...
{$ENDIF}
  ...
  BindingBlendPixel := TFunctionBinding.Create(@@BlendPixel, @BlendPixelNative);
...
{$ENDIF}
  ...
  BindingBlendPixelInplace := TFunctionBinding.Create(@@BlendPixelInplace,
...
{$ENDIF}
  ...
  BindingBlendPixelLine := TFunctionBinding.Create(@@BlendPixelLine,
...
{$ENDIF}
  ...
  // create function binding for blend line
  BindingBlendLine := TFunctionBinding.Create(@@BlendLine, @BlendLineNative);
...
{$ENDIF}
  ...
  BindingCombinePixel := TFunctionBinding.Create(@@CombinePixel,
    @CombinePixelNative);
...
{$ENDIF}
  ...
  BindingCombinePixelInplace := TFunctionBinding.Create(@@CombinePixelInplace,
...
{$ENDIF}
  ...
  // create function binding for combine memory
  BindingCombinePixelLine := TFunctionBinding.Create(@@CombinePixelLine,
  ...
  BindingCombineLine := TFunctionBinding.Create(@@CombineLine,
    @CombineLineNative);
...
{$ENDIF}
  ...
  // create function binding for combine register
  BindingMergePixel := TFunctionBinding.Create(@@MergePixel, @MergePixelNative);
  ...
  BindingMergePixelInplace := TFunctionBinding.Create(@@MergePixelInplace,
  ...
  BindingMergeLine := TFunctionBinding.Create(@@MergeLine, @MergeLineNative);
